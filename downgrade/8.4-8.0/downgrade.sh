...
if [ "$NUMBER_OF_SITES" -gt 0 ]; then
  echo "MySQL 8.0 cannot be installed when sites are present." >&2
  exit 1
fi
...
sqlite3 /home/clp/htdocs/app/data/db.sq3 "UPDATE database_server SET version = '8.0' WHERE host = '127.0.0.1';"